...
from model import DDQN, CnnDDQN
...
class Agent:
    def __init__(self, env, use_cnn=False, learning_rate=3e-4, gamma=0.99, buffer_size=10000):
        ...
        self.ddqn = CnnDDQN(env.observation_space.shape, env.action_space.n) if use_cnn else DDQN(env.observation_space.shape[0], env.action_space.n) 
        self.ddqn_optimizer = torch.optim.Adam(self.ddqn.parameters())
        self.ddqn_loss = torch.nn.MSELoss()
